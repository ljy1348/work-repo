-- sql developer 툴 : DB 접속 툴(DB 클라이언트 툴)
-- oracle 설치 : DB 서버
...
-- SQL 질의문(query)특징 : 대소문자 구문이 없음(오라클 DB : 대문자가 성능이 빠름)
-- DB(데이터베이스) : 데이터를 관리 또는 저장하기 위한 최소단위 : 테이블

-- 1) SQL 문 : 검색(조회)
-- 사용법) SELECT 컬럼명(* : 전체선택) FROM 테이블명
...
-- 예제 2) 사원 테이블의 컬럼중에서 ENO, ENAME 컬럼만 정보를 보는 SQL 문을 작성하세요
SELECT ENO,ENAME FROM EMPLOYEE;

-- 예제 3) 부서테이블의 컬럼중에서 DNO, LOC 컬럼만 조회
SELECT DNO,LOC FROM DEPARTMENT;

-- 예제 4) 힌트) 컬럼에 산술 연산자로 연산해서 화면에 출력할 수 있다.
-- 사원 테이블 : EMPLOYEE
-- 월급 컬럼 : SALARY
-- 사원명 컬럼 : ENAME
-- 사원 테이블에서 사원명과 1년 연봉을 화면에 출력해 보세요
...
-- 예제 5-2)
-- 위의 데이터 값을 보정하기(null 보정)
-- NULL 특징 : NULL 과 산순열산을 할 수 없음 -> 결과가 NULL로 나옴
-- 사용법 : NVL(NULL값이 있는 컬럼, 변경될 값)
...
-- 활용법
SELECT 4*5+1 FROM DUAL; -- <- 간단한 계산 용도 출력 : 21
-- 문자열 : ''(홑따옴표) 사용
SELECT '홍길동' FROM DUAL; -- 텍스트 출력 출력 : 홍길동
-- 오늘 날짜 : SYSDATE 속성
