...
-- 연습문제 4) 관리자 수를 출력하세요. (매니저 컬럼의 데이터 갯수를 나열하세요.)
...
SELECT DNO AS 사원번호, ROUND(AVG(SALARY),2)AS 평균급여
        ,COUNT(DNO) AS 사원수
FROM EMPLOYEE
GROUP BY DNO;
...
SELECT DNO, ROUND(AVG(SALARY)) AS AVG_SAL
...
-- 연습문제 10 사원들의 입사 연도를 기준으로 부서별로 몇명이 입사했는지 출력하세요.
