-- 연습 문제
-- 1) SUBSTR 함수를 사용하여 사원들의 입사 년도와 월만 출력하세요.
-- SUBSTR (컬럼명, 인덱스 위치, 잘라낼 갯수)
...
-- 단, 포맷은 'YYYY/MM/DD' (참고)
SELECT (TRUNC(SYSDATE - TO_DATE('2023/01/01', 'YYYY/MM/DD'))) AS 지난날짜
FROM DUAL;

-- 4) 사원들의 매니저 사번을 출력하되 매니저가 없는 사원에 대해서는 NULL 대신에 0을 출력하세요
...
SELECT ENO, ENAME, JOB, SALARY
      ,DECODE(JOB, 'ANALYST', SALARY+200
                    ,'SALESMAN', SALARY+180
                    ,'MANAGER', SALARY+150
                    ,'CLERK', SALARY+100, SALARY) AS 급여인상
FROM EMPLOYEE;
...
SELECT ENO,ENAME,NVL(TO_CHAR(COMMISSION), 'N/A') AS 상여금
        ,TO_CHAR(NEXT_DAY(ADD_MONTHS(HIREDATE, 3),'월요일'), 'YYYY-MM-DD') AS 정직원
FROM EMPLOYEE;
